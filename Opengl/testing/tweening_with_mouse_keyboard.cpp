...
void mouse(int button,int state,int x, int y){
	...
	if(button ==GLUT_LEFT_BUTTON && state==GLUT_DOWN){
		...
		for(int i=0;i<t1.get_triangle_size();i++){
			glVertex2f(t1.tri[i].x+x1,t1.tri[i].y+y1);
			}
		glEnd();
		glFlush();
	}
	if(button==GLUT_RIGHT_BUTTON && state== GLUT_DOWN){
		...
		glBegin(GL_TRIANGLES);
		for(int i=0;i<t2.get_triangle_size();i++){
			glVertex2f(t2.tri[i].x+x1,t2.tri[i].y+y1);
			}
		glEnd();
		...
		glBegin(GL_TRIANGLES);
			//for(float i=0;i<=1;i=i+0.4){
				for(float j=0;j<t1.get_triangle_size();j++){
					float x = t1.tri[j].x+(t2.tri[j].x-t1.tri[j].x)*position;
					float y = t1.tri[j].y+(t2.tri[j].y-t1.tri[j].y)*position;
					glVertex2f(x,y);
